...
def insert_into_db(host, user, password, db, rate):
    connection = pymysql.connect(host=host, user=user, password=password, db=db)
    try:
        with connection.cursor() as cursor:
            # Create a new record
            sql = "INSERT INTO `plnpkr` (`pln`, `pkr`, `time`) VALUES (%s, %s, %s)"
...
if isinstance(exchange_rate, float):
    ...
    insert_into_db(db_host, db_user, db_password, db_name, exchange_rate)
